...
4. Distinguish local test coverage from shared-infra or runtime-artifact
   evidence.
5. Fail loudly when a closed row lacks its declared evidence kind.
6. Surface stale or contradictory active docs.
7. Produce human-readable and agent-readable reports.
...
- The checker catches missing files, missing test refs, missing shared-infra
  owners, and closed rows without required evidence.
...
Advances: convert the current read model into governed data.
