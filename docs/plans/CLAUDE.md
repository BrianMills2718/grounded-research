...
| [tyler_literal_parity_refactor.md](tyler_literal_parity_refactor.md) | In Progress | Major contract migration to make Tyler's V1 schemas and prompts literal repo-local runtime surfaces rather than adapted references. Runtime Stage 1-6 migration is now done; the remaining open work is benchmark re-anchor and explicit shared-infra boundary recording. |
...
| [tyler_literal_parity_benchmark_reanchor.md](tyler_literal_parity_benchmark_reanchor.md) | In Progress | Post-migration benchmark gate for the fully Tyler-native runtime: complete one live Stage 1-6 trace, rerun the tracked benchmark, and classify any remaining divergence as repo-local or shared-infra. |
