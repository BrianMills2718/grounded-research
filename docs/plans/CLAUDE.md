...
| [tyler_literal_parity_refactor.md](tyler_literal_parity_refactor.md) | In Progress | Major contract migration to make Tyler's V1 schemas and prompts literal repo-local runtime surfaces rather than adapted references. Stages 4-6 runtime parity are now in; the remaining gap is replacing Stage 1-3 adapter-fed inputs with native Tyler runtime artifacts and re-anchoring benchmarks. |
| [tyler_literal_parity_stage123_native.md](tyler_literal_parity_stage123_native.md) | In Progress | Execution-ready remaining wave for literal Tyler parity: persist Tyler Stage 1-3 artifacts, switch those stages to native runtime production, and re-anchor Stage 4-6 on the persisted artifacts. |
