...
| [isolated_compatibility_surface_deletion.md](isolated_compatibility_surface_deletion.md) | In Progress | Current child wave under Tyler canonical cutover: delete non-live compatibility APIs (`decompose_with_validation`, legacy analyst path, current-shape Stage 3/3a helpers) so `main` keeps one canonical runtime vocabulary. |
