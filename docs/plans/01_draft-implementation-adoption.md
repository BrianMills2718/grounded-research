# Gap 1: Draft Implementation Adoption Gate
...
**Current:** The worktree contains untracked implementation drafts
(`engine.py`, `config/`, `prompts/`, `src/grounded_research/*.py`) that go
beyond the last accepted documentation milestone.

**Target:** An explicit decision for each draft surface: `accept`, `hold`, or
`discard`, with the rationale persisted in docs before any implementation is
treated as project state.

**Why:** Without an adoption gate, local drafts silently compete with the
canonical plan and create confusion about what the repo has actually decided.
...
1. Inventory the local draft implementation surfaces.
2. Compare each draft against the canonical plan and contracts.
3. Record for each draft whether it should be accepted now, held for later, or
   discarded.
...
This is a review-and-planning gate, not an implementation milestone.
