...
|---|---|---|---|---|---|---|---|---|
| BP-INTRO-001 | `1. V1_Build_Plan_Step_By_Step.md` | `# V1 Build Plan — Step by Step` | `context_only` | overall execution intent and phase decomposition | static review | pending | none | root source summary |
| BP-S1-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Stage 1 — Intake & Decomposition` | `stage_rule` | Stage 1 runtime behavior, outputs, and limits | static + runtime | pending | none | split into subunits as needed |
| BP-S2-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Stage 2 — Broad Retrieval & Evidence Normalization` | `stage_rule` | Stage 2 retrieval, routing, scoring, normalization | static + runtime | pending | none | split into subunits as needed |
| BP-S3-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Stage 3 — Independent Candidate Generation` | `stage_rule` | Stage 3 model roles, quality floor, outputs | static + runtime | pending | none | split into subunits as needed |
| BP-S4-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Stage 4 — Claim Extraction & Dispute Localization` | `stage_rule` | Stage 4 extraction, localization, ordering | static + runtime | pending | none | split into subunits as needed |
| BP-S5-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Stage 5 — Targeted Verification & Arbitration` | `stage_rule` | Stage 5 query roles, search params, arbitration, caps | static + runtime | pending | none | split into subunits as needed |
| BP-S6A-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Stage 6a — User-Steering Interrupt` | `branch_or_skip_rule` | Stage 6a trigger, queue selection, surfaced disputes | static + runtime | pending | none | split into subunits as needed |
| BP-S6B-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Stage 6b — Synthesis & Final Report` | `output_contract` | Stage 6 synthesis, evidence use, output requirements | static + runtime | pending | none | split into subunits as needed |
...
| BP-TRACE-001 | `1. V1_Build_Plan_Step_By_Step.md` | `### Trace File (separate from report)` | `output_contract` | trace file fields and behavior | static + artifact | pending | none |  |
| BP-CCR-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Cross-Cutting Requirements` | `cross_cutting_rule` | umbrella cross-cutting rules | static review | pending | none | split to subunits below |
| BP-CCR-REASONING-001 | `1. V1_Build_Plan_Step_By_Step.md` | `### Reasoning field` | `cross_cutting_rule` | reasoning field protocol | static review | pending | none |  |
| BP-CCR-FALLBACK-001 | `1. V1_Build_Plan_Step_By_Step.md` | `### Fallback logic` | `cross_cutting_rule` | fallback behavior constraints | static + runtime | pending | none |  |
| BP-CCR-PARTIALTRACE-001 | `1. V1_Build_Plan_Step_By_Step.md` | `### Partial trace on abort` | `cross_cutting_rule` | abort-time trace preservation | static + failure-path | pending | none |  |
| BP-CCR-ANON-001 | `1. V1_Build_Plan_Step_By_Step.md` | `### Anonymization` | `cross_cutting_rule` | anonymization requirements | static + runtime | pending | none |  |
| BP-CCR-ANTICONFORMITY-001 | `1. V1_Build_Plan_Step_By_Step.md` | `### Anti-conformity` | `cross_cutting_rule` | anti-conformity enforcement | static + runtime | pending | none |  |
| BP-LIMITATIONS-001 | `1. V1_Build_Plan_Step_By_Step.md` | `## Known Limitations` | `context_only` | accepted limitations and non-goals | static review | pending | none | classify any normative caveats separately if needed |
...
| SC-S2-001 | `3. V1_SCHEMAS.md` | `## Stage 2 — Broad Retrieval & Evidence Normalization` | `schema_contract` | Stage 2 schema | static + tests | pending | none |  |
...
| PR-GLOBAL-001 | `4. V1_PROMPTS.md` | `## Global Prompt Conventions [GPT]` | `prompt_template` | global prompt conventions | prompt render + static review | pending | none |  |
| PR-FRAMES-001 | `4. V1_PROMPTS.md` | `## Reasoning Frame Assignments` | `model_role_rule` | frame-to-model prompt assignment | static + runtime | pending | none |  |
| PR-INVENTORY-001 | `4. V1_PROMPTS.md` | `## Prompt Inventory` | `context_only` | prompt inventory completeness | static review | pending | none |  |
...
| PR-S2-QUERYTEMPLATE-001 | `4. V1_PROMPTS.md` | `## Stage 2 — Query Variant Generation (Orchestrator Template)` | `orchestrator_template` | Stage 2 query variant template | static + runtime | pending | none |  |
...
| PR-NOTES-SHAREDOUTPUT-001 | `4. V1_PROMPTS.md` | `## Notes for Developer / ### Shared Output Protocol` | `cross_cutting_rule` | shared output protocol | static review | pending | none |  |
