...
| Exact Tyler Gemini model-version parity | shared model registry + config surfaces | Tyler names Gemini 3.1 Pro, while the live config still uses `openrouter/google/gemini-2.5-pro`. OpenRouter currently exposes `google/gemini-3.1-pro-preview`, but `llm_client`'s shared default model registry does not yet expose that Gemini 3.1 Pro surface and `grounded-research` therefore still documents the substitution rather than claiming literal parity |
