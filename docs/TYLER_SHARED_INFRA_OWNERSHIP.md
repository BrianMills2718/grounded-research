...
| Gemini strict-schema quality study | `llm_client` + `prompt_eval` | active shared closure path now exists as `llm_client` PR #27 (`gemini-schema-main-merge` @ `e9a0cbf`): Plan 26 recorded the direct-vs-OpenRouter evidence and Plan 27 replaced the hardcoded direct-Gemini `budget_tokens=0` default with shared config; PR is mergeable and focused tests passed, but repo auto-merge is disabled so the remaining step is manual merge plus any follow-on `prompt_eval` use |
| Exact frontier-model role parity from Tyler's original stack | shared model availability + config surfaces | three literal production-config runs are now recorded; Frontier Reliability Wave 3 showed the failed PFAS run was a clean Claude Opus Stage 3 response with one uncited claim, while the next PFAS repeat and the Palantir run passed. The remaining issue is now best classified as model-output variability / model-policy limitation rather than a `llm_client` transport defect or a repo-local bug |
